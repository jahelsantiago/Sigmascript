...
class Model:
        ...
        # recuento de todos los datos del bloque b-esimo
            ...
            if b <= len(self.b) - 1:
                ...
                print(
                    "El bloque numero "
                    + str(b)
                    + " no existe, recuerde que la numeración comienza en 0"
                )
    ...
    def findElementSymbol(
        self, element
    ):  # retorna simbolo relacionado al tratamiento que pertenece
    ...
    def multicotomize(
        self, numberList
    ):  # multicotomiza el bloque i-esimo, retorna chain
    ...
    def toStreakModel(self):  # algortimo de multicomozación del modelo de 2 vias
        ...
        for i in range(len(self.M)):
            ...
            NumberofStreaks = chain.numberOfStreaks()
            ...
            for j in range(len(self.M[i])):
                for k in range(len(self.M[i][j])):
                    if positionsList[count] < len(chain.chainArray) - 1:
                        ...
                        self.Z[i][j][k] = chain.numberOfStreaksUntilIndex(
                            positionsList[count]
                        )
                        # print(self.Z[i][j])
                        count += 1
                        ...
                        count += 1
                        self.Z[i][j][k] = chain.numberOfStreaks()
                    # print(self.Z[i][j])

        streakModel = StreakModel(self.Z)
        return streakModel
...
class StreakModel:
    ...
    def streakOperationSum(
        self, i, j, k
    ):  # operaciones de SUMA sobre modelo de rachas, 5 operaciones
    ...
    def StreakOperationAvrg(
        self, i, j, k
    ):  # operaciones de PROMEDIO sobre modelo de rachas, 4 operaciones
...
class Chain:
    ...
    def numberOfStreaksUntilIndex(self, index):
...
# print(cadena.numberOfStreaksUntilIndex(3))


# modelodeRachas = modeloZ.toStreakModel()

# print(modelodeRachas.Z)

# print(modelodeRachas.streakOperationSum(".",2,"."))

# print(modelodeRachas.StreakOperationAvrg(0,2,"."))
